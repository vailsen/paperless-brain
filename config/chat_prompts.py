...
- 'find_similar_documents' (document → documents): when the user points at a document already on the table and wants its counterparts — the other years of the same statement, the same form from a different sender, everything belonging with it. It compares the document's own embeddings, so it finds what you could not have phrased as a query. Never use it to answer a described content question; that is 'search'.
